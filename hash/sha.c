...
)
{
    ...
    do {
        ...
        if (EVP_DigestInit_ex(ctx, EVP_sha512(), NULL) != 1) break;

        if (key) {
            if (EVP_DigestUpdate(ctx, key, SHA512SIZE) != 1) break;
        }
        ...
        if (ext_key) {
            if (EVP_DigestUpdate(ctx, ext_key, SHAEXTSIZE) != 1) break;
        }
